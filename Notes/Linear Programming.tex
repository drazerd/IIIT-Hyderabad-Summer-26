...
\[
    ...
    \end{aligned}
\]
where $y_i$ are the dual variables associated with the equality constraints, and $z_j$ are the dual variables associated to the inequality constraints.
...
A priori the dual variables are arbitrary, however we constrain the variable $z_j$ concerned with the inequality constraints, to be non-negative
$$z_j \ge 0  \forall  j.$$
...
The main observation now is that the tightest upper bound on the primal objective function is obtained by minimising the lagrangian over the dual variables subject to the constraints, that we imposed on $z_j$ and $y_i$.
...
\[
    \begin{aligned}
        \text{minimise} \quad   & \sum_{i=1}^{m} b_i y_i + \sum_{j=1}^{n} c_j z_j                                     \\[0.5em]
        \text{subject to} \quad & \sum_{i=1}^{m} y_i r_{ik} + \sum_{j=1}^{n} z_j s_{jk} \geq a_k, \qquad \forall\, k, \\[0.5em]
...
\[
    \begin{aligned}
        \text{minimise} \quad   & \vec{b} \cdot \vec{y} + \vec{c} \cdot \vec{z} \\[0.5em]
        \text{subject to} \quad & B^T \vec{y} + C^T \vec{z} = \vec{a},          \\[0.5em]
                                & \vec{z} \ge \vec{0}.
    \end{aligned}
\]
...
The \textit{dual objective function}, is linear in the dual variables $\vec{y}$ and $\vec{z}$ with the inequality constraint, $\vec{z} \ge 0$. Notice that in the dual variable problem we have two variables instead of one, and this can be resolved by concatenating the vectors $\vec{y}$ and $\vec{z}$ in a single vector,
$\vec{y'} = (\vec{y}, \vec{z}) \in \mathbb{R}^{m+n}.$

\subsection{Dimensions of the dual problem}
There is a clear difference between the dimensions of the primal and the dual problem. The primal problem optimises over $k$ variables and involves $m$ equality and $n$ inequality constraints; the dual problem optimises over $n+m$ variables and involves $k$ equality and $n$ inequality constraints.
...
\begin{align}
     & \beta = \min_{(\vec{y},  \vec{z}) \in \tilde{\mathcal{F}}} \vec{b} \cdot \vec{y} + \vec{c} \cdot \vec{z}.
\end{align}

Similar to the primal LP, any pair of variables $(\vec{y}^{*}, \vec{z}^{*})$ that achieve the minimum are called \textit{dual optimal variables}.
...
Note that any feasible pair of dual variables $\vec{y}$ and $\vec{z}$, satisfying the constraints of the dual LP, provide an upper bound on the dual optimal value $\beta$, similarly any feasible primal variable $\vec{x}$ satisfying the constraints of the primal LP, provides a lower bound on the optimal value $\alpha$.
We introduced the dual LP in order to obtain the tightest upper bound on the primal optimal value.
\begin{align}
...
This relation is known as \textit{weak duality.} Combining the above statements, we can see that
$$\vec{a}\cdot \vec{x} \le \alpha \le \beta \le \vec{b}\cdot \vec{y} + \vec{c}\cdot \vec{z}$$
for all primal feasible points $\vec{x}$ and dual feasible pairs $\vec{y}$ and $\vec{z}$. It is straightforward to realise that if one finds primal and dual feasible variables such that
$$ \vec{a}\cdot \vec{x} = \vec{b} \cdot \vec{y} + \vec{c}\cdot \vec{z}$$
...
Strong duality is the generic behaviour of the LPs; in practice it is essentially always the case that an LP will satisfy strong duality, in particular it can be shown that
